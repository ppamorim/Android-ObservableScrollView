...
public abstract class FillGap3BaseRecyclerActivity<S extends RecyclerViewScrollable> extends BaseActivity implements
  ...
  private TouchInterceptionFrameLayout.TouchInterceptionListener mInterceptionListener = new TouchInterceptionFrameLayout.TouchInterceptionListener() {
    ...
    public boolean shouldInterceptTouchEvent(MotionEvent ev, boolean moving, float diffX, float diffY) {
      return getMinInterceptionLayoutY() < (int) ViewHelper.getY(mInterceptionLayout) || (moving && mScrollable.getCurrentScrollY() - diffY < 0);
    }
  ...
  protected void updateViews(float translationY, boolean animated) {
    ...
    ViewHelper.setTranslationY(mImageView,
        (translationY - (mFlexibleSpaceImageHeight - mHeaderBarHeight)) / 2);
